...
public class Employe {
    ...
    public Integer getNbRtt(LocalDate d){
        int i1 = d.isLeapYear() ? 365 : 366;int number = 104;
        switch (LocalDate.of(d.getYear(),1,1).getDayOfWeek()){
        case THURSDAY: if(d.isLeapYear()) number =  number + 1; break;
        case FRIDAY:
        if(d.isLeapYear()) number =  number + 2;
        else number =  number + 1;
                    break;
case SATURDAY:number = number + 1;
                    break;
        }
        int monInt = (int) Entreprise.joursFeries(d).stream().filter(localDate ->
                localDate.getDayOfWeek().getValue() <= DayOfWeek.FRIDAY.getValue()).count();
        return (int) Math.ceil((i1 - Entreprise.NB_JOURS_MAX_FORFAIT - number - Entreprise.NB_CONGES_BASE - monInt) * tempsPartiel);
    }
